...
test("handoff bridge becomes a clean Google Chat tab after transferring the port", () => {
  ...
  assert.equal(calls.workerMessages[0].releaseVersion, "0.3.4");
  assert.equal(calls.workerMessages[0].platform, "googleChat");
  assert.equal(calls.workers[0].url, "./chatbut-bridge-worker.js?v=0.3.4");
  assert.equal(calls.workers[0].options.name, "chatbut-config-bridge-v0.3.4");
});
